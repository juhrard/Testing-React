...
test('Renders a list of 10 Star Wars Characters and 2 navigation buttons', async () => {
  ...
  const newCharacters = findAllByTestId('test');

  wait(() => expect(newCharacters.length === 10).toBeTruthy());
...
test('Clicking Navigation Buttons returns a value for Previous', async () => {
  ...
  const prevButton = findByText('Previous');
  const nextButton = findByText('Next');

  wait(() => fireEvent.click(prevButton));
  wait(() => {
    fireEvent.click(nextButton);
    expect(mockCharacters.previous).toBeTruthy();
  });
